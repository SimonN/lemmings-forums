...
if ((!isset($modSettings['smfVersion']) || $modSettings['smfVersion'] <= '1.1 RC1') && empty($modSettings['dont_repeat_theme']))
{
	...
	if (mysqli_num_rows($theme_request) == 0)
	{
		...
		{
			$core = mysqli_fetch_assoc($request);
		...
		$request = upgrade_query("
			...
			FROM {$db_prefix}themes");
		list ($ID_OLD_THEME) = mysqli_fetch_row($request);
		mysqli_free_result($request);
	...
	while ($_GET['lt'] <= $maxTopics)
	{
		...
		while ($row = mysqli_fetch_assoc($request))
		{
			foreach (text2words($row['subject']) as $word)
				$inserts[] = "'" . mysqli_real_escape_string($word) . "', $row[ID_TOPIC]";
		}
		mysqli_free_result($request);
